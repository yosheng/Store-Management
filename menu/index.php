...
if (isset($_GET["action"]) && ($_GET["action"] == "delete")) {
  ...
  //刪除所屬庫存
  $query_delphoto = "SELECT * FROM item WHERE `category_id`=" . $_GET["id"];
  $delphoto = mysql_query($query_delphoto);
  while ($row_delphoto = mysql_fetch_assoc($delphoto)) {
    unlink("photos/" . $row_delphoto["ap_picurl"]);
  }
  //刪除庫存
  $query_del1 = "DELETE FROM `category` WHERE `category_id`=" . $_GET["id"];
  $query_del2 = "DELETE FROM `item` WHERE `category_id`=" . $_GET["id"];
...
<head>
  ...
  initial-scale=1.0,
  maximum-scale=1.0,
  user-scalable=no"><!--控制顯示大小-->
  <title>商品庫存管理系統-管理庫存</title>
  ...
  <link rel="icon" href="../favicon.ico"><!--匯入icon-->
...
<body topmargin="0" leftmargin="0" marginwidth="0" marginheight="0"><!--body頁面邊框設定-->
 <!-- NAV BAR 專用-->
<?php
          ...
          <table width="90%" border="0" align="center" cellpadding="4" cellspacing="0" ><!--連結配置-->
           <tr valign="top">
             <td>
               ...
               <div class="col-lg-4">
                 <div class="thumbnail">
                   <a href="itemfix.php?id=<?php echo $row_RecAlbum["category_id"];?>"><?php if ($row_RecAlbum["albumNum"] == 0) {?>
                     <img src="images/nopic.png" alt="暫無圖片" style="width:250px;height:250px" /><?php } else {?>
                     <img src="shops/<?php echo $row_RecAlbum["ap_picurl"];?>" alt="<?php echo $row_RecAlbum["category_title"];?>" style="width:250px;height:250px" />
                     <?php }?>
                   </a>
                 </div>
                 ...
                 <div class="row" align="center">
                   <label class="control-label col-lg-12 labelstyle">
                     <a href="itemfix.php?id=<?php echo $row_RecAlbum["category_id"];?>"><?php echo $row_RecAlbum["category_title"];?></a>
                   </label>
                 ...
                 <div class="row" align="center">
                   <label class="control-label col-lg-12 labelstyle">
                     <a href="?action=delete&id=<?php echo $row_RecAlbum["category_id"];?>" class="smalltext" onClick="javascript:return deletesure();">(刪除庫存)</a>
                   </label>
